...
def draw_name(win, player):
    ...
    while draw:
        ...
        lost_text = TITLE_FONT.render('GAME OVER!', True, TEXT_COLOR)
...
def main_menu(win):
    ...
    while run:
        win.fill(BACKGROUND_COLOR)
        buttons = ['START NEW GAME', f'SPEED: {speeds[player.speed]}', f'Guide: {modes[mode]}', 'SCORES', 'QUIT']
        draw_menu(win, 'MAIN MENU - TETRIS GAME', buttons, WIDTH, HEIGHT, active)
        pygame.display.update()
